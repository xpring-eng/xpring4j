  ...
  public static Collection<Object[]> data() {
    ...
    return Arrays.asList(new Object[][] {
      ...
      //0
      {
        ...
        "Pay ID must not be null", // message to expect
      },
      //1
      {
        "$", // input (missing host)
        IllegalArgumentException.class, // exception to expect
        "Pay ID `$` must start with the 'payid:' scheme", // message to expect
      },
      //2
      {
        "payid:$", // input (missing host)
        IllegalArgumentException.class, // exception to expect
        "Pay ID `$` must specify a valid account and host", // message to expect
      },
      //3
      {
        "alice$wallet.example", // input missing payid: prefix
        IllegalArgumentException.class, // exception to expect
        "Pay ID `alice$wallet.example` must start with the 'payid:' scheme", // message to expect
      },
      //4
      {
        "payid:alice$wallet.example$bank.example", // input with two dollar-signs
        IllegalArgumentException.class, // exception to expect
        "Pay ID `alice$wallet.example$bank.example` may only contain a single dollar-sign. "
      ...
      //20 (don't accept payid that start with a %-encoded value.)
      {
        ...
        IllegalArgumentException.class, // exception to expect
        "Pay ID `%20abc$@example.com` MUST start with either an 'unreserved' or 'sub-selims' character rules. "
          + "A Pay ID may not start with a percent-encoded value.", // message to expect
      },
